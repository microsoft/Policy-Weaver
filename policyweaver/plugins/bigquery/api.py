...
from policyweaver.plugins.bigquery.model import (
    BigQueryConnection,
    BigQueryGrant,
    BigQueryRole,
    BigQueryRoleMemberMap,
    BigQueryUser,
    BigQueryDatabaseMap,
    BigQueryUserOrRole
)
...
class BigQueryAPIClient:
    """
    BigQuery API Client for fetching account policies.
    This client uses the BigQuery API to interact with the BigQuery account
    ...
    from BigQuery workspaces and accounts.
    """
    def __init__(self):
        """
        Initializes the BigQuery API Client with a connection to the BigQuery account.
    ...
    def __get_database_map__(self, source: Source) -> BigQueryDatabaseMap:
        """
        Retrieves the database map from the BigQuery account.
        Returns:
        ...
        return BigQueryDatabaseMap(users=self.users, roles=self.roles, grants=self.grants)

    def __get_grants__(self, source: Source) -> List[BigQueryGrant]:
        ...
        # grants = [BigQueryGrant(privilege=grant["PRIVILEGE"],
    ...
    def __get_user_role_assignment__(self, user: BigQueryUserOrRole, is_user: bool) -> List[BigQueryRole]:
    ...
    def __get_user_role_assignments__(self) -> dict[str, List[BigQueryRole]]:
    ...
    def __get_role_memberships__(self) -> dict[str, BigQueryGrant]:
        ...
        for role in self.roles:
            ...
            role_memberships[role_name] = BigQueryRoleMemberMap(
                role_name=role_name,
                users=member_users,
                roles=member_roles
            )
    ...
    def __get_users__(self) -> List[BigQueryUser]:
    ...
    def __get_roles__(self) -> List[BigQueryRole]:
        """
        Retrieves all roles from the BigQuery account.
        Returns:
            List[BigQueryRole]: A list of BigQueryRole objects representing the roles in the account.
        """
